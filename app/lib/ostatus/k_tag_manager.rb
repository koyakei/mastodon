...
class OStatus::KTagManager
  ...
  def unique_tag(date, id, type)
    "k_tag:#{Rails.configuration.x.local_domain},#{date.strftime('%Y-%m-%d')}:objectId=#{id}:objectType=#{type}"
  end
  ...
  def local_id?(id)
    id.start_with?("k_tag:#{Rails.configuration.x.local_domain}") || ActivityPub::TagManager.instance.local_uri?(id)
  end
