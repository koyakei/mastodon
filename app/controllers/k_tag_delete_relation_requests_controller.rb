class Api::V1::KTagDeleteRelationRequestsController < Api::BaseController
  ...
  def create
    logger.info "daadfsapi_v1_k_tag_delete_relation_request_params: #{api_v1_k_tag_delete_relation_request_params.inspect}"
    ...
    if k_tag_relation.nil?
      ...
      if k_tag_relation.destroy
        ...
        render json: k_tag_relation.status, status: 200, serializer: REST::StatusSerializer
      else
        render json: k_tag_relation.status, status: 404, serializer: REST::StatusSerializer
      end
    else
      # 他人の所有しているタグだった場合リクエストを送る　通知
      api_v1_k_tag_delete_relation_request = KTagDeleteRelationRequest.new(
        api_v1_k_tag_delete_relation_request_params.merge(requester_id: current_user.account_id, k_tag_relation_id: k_tag_relation.id)
      )
      logger.info "KTagDeleteRelationRequest: #{api_v1_k_tag_delete_relation_request.inspect}"
      if api_v1_k_tag_delete_relation_request.save # 　すでに削除されている場合と、すでに削除リクエストがされている場合をわけてレスポンスしたい
        ...
        render json: k_tag_relation.status, status: 200, serializer: REST::StatusSerializer, status: :accepted
      elsif api_v1_k_tag_delete_relation_request.errors.of_kind?(:k_tag_relation_id, :taken)
        render json: { errors: api_v1_k_tag_delete_relation_request.errors.full_messages }, status: 409
      else
  ...
  def approve
    ...
    if @api_v1_k_tag_delete_relation_request.approved?
      ...
      render json: { error: 'k tag relation alredy discarded' }, status: :unprocessable_entity if k_tag_relation.discarded?
      ...
      render json: @api_v1_k_tag_delete_relation_request, status: 200
    end
  end
  ...
  def deny
    ...
    elsif @api_v1_k_tag_delete_relation_request.update(request_status: :denied, review_comment: paarams[:review_comment] || '')
  ...
  def destroy
    ...
    redirect_to api_v1_k_tag_add_relation_request_url, notice: 'K tag add relation reque was successfully destroyed.', status: 303
  end
